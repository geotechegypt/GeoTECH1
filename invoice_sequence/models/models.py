...
class invoice_sequence(models.Model):
    _inherit = 'account.move'
    @api.depends('posted_before', 'state', 'journal_id', 'date')
    def _compute_name(self):
        for r in self:
            if r.move_type != 'entry':
                for invoice in self:
                    if not self.name:
                        if r.move_type != 'entry' and invoice.move_type == 'out_invoice':
                            invoice.name = self.env['ir.sequence'].next_by_code('invoice')
                        elif r.move_type != 'entry' and invoice.move_type == 'out_refund' :
                            invoice.name = self.env['ir.sequence'].next_by_code('refund')
                        elif r.move_type != 'entry' and invoice.move_type == 'in_refund':
                            invoice.name = self.env['ir.sequence'].next_by_code('refund_bill')
                        elif r.move_type != 'entry' and invoice.move_type == 'in_invoice':
